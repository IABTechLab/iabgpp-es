...
describe("manifest.section.UsRi", (): void => {
  it("should encode default", (): void => {
    let usRi = new UsRi();
    expect(usRi.encode()).to.eql("BQAAAAA.QA");
  });
  ...
  it("should decode", (): void => {
    ...
    expect(usRi.getFieldValue(UsRiField.GPC)).to.eql(true);
  });
