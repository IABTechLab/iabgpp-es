...
export class UsCtV1CoreSegment extends AbstractLazilyEncodableSegment<EncodableBitStringFields> {
  ...
  public validate(): void {
    let saleOptOutNotice: number = this.fields.get(UsCtV1Field.SALE_OPT_OUT_NOTICE).getValue();
    let saleOptOut: number = this.fields.get(UsCtV1Field.SALE_OPT_OUT).getValue();
    let targetedAdvertisingOptOutNotice: number = this.fields
      .get(UsCtV1Field.TARGETED_ADVERTISING_OPT_OUT_NOTICE)
      .getValue();
    let targetedAdvertisingOptOut: number = this.fields.get(UsCtV1Field.TARGETED_ADVERTISING_OPT_OUT).getValue();
    let mspaServiceProviderMode: number = this.fields.get(UsCtV1Field.MSPA_SERVICE_PROVIDER_MODE).getValue();
    let mspaOptOutOptionMode: number = this.fields.get(UsCtV1Field.MSPA_OPT_OUT_OPTION_MODE).getValue();
