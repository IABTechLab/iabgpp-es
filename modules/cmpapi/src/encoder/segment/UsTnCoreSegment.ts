...
export class UsTnCoreSegment extends AbstractLazilyEncodableSegment<EncodableBitStringFields> {
  ...
  public validate(): void {
    let saleOptOutNotice: number = this.fields.get(UsTnField.SALE_OPT_OUT_NOTICE).getValue();
    let saleOptOut: number = this.fields.get(UsTnField.SALE_OPT_OUT).getValue();
    let targetedAdvertisingOptOutNotice: number = this.fields
      .get(UsTnField.TARGETED_ADVERTISING_OPT_OUT_NOTICE)
      .getValue();
    let targetedAdvertisingOptOut: number = this.fields.get(UsTnField.TARGETED_ADVERTISING_OPT_OUT).getValue();
    let mspaServiceProviderMode: number = this.fields.get(UsTnField.MSPA_SERVICE_PROVIDER_MODE).getValue();
    let mspaOptOutOptionMode: number = this.fields.get(UsTnField.MSPA_OPT_OUT_OPTION_MODE).getValue();
    ...
    if (targetedAdvertisingOptOutNotice == 0) {
      if (targetedAdvertisingOptOut == 1 || targetedAdvertisingOptOut == 2) {
        throw new ValidationError(
          "Invalid ustn targeted advertising notice / opt out combination: {" +
    ...
    if (mspaServiceProviderMode == 0) {
      if (saleOptOutNotice == 1 || saleOptOutNotice == 2) {
        throw new ValidationError(
          "Invalid ustn mspa service provider mode / sale opt out notice combination: {" +
      ...
      if (mspaOptOutOptionMode == 0 || mspaOptOutOptionMode == 1) {
        throw new ValidationError(
          "Invalid ustn mspa service provider / opt out option modes combination: {" +
            mspaServiceProviderMode +
            " / " +
            mspaServiceProviderMode +
            "}"
