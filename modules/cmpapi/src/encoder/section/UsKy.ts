...
import { UsKyGpcSegment } from "../segment/UsKyGpcSegment.js";
...
export class UsKy extends AbstractLazilyEncodableSection {
  ...
  protected initializeSegments(): EncodableSegment[] {
    ...
    segments.push(new UsKyGpcSegment());
    return segments;
  }
  ...
  protected decodeSection(encodedString: string): EncodableSegment[] {
    ...
    if (encodedString != null && encodedString.length !== 0) {
      ...
      if (encodedSegments.length > 1) {
        segments[1].setFieldValue(UsKyField.GPC_SEGMENT_INCLUDED, true);
        segments[1].decode(encodedSegments[1]);
      } else {
        segments[1].setFieldValue(UsKyField.GPC_SEGMENT_INCLUDED, false);
      }
    }
  ...
  protected encodeSection(segments: EncodableSegment[]): string {
    ...
    if (segments.length >= 1) {
      ...
      if (segments.length >= 2 && segments[1].getFieldValue(UsKyField.GPC_SEGMENT_INCLUDED) === true) {
